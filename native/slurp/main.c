...
/* Snapshot only modifiers, before keyboard focus returns to another client. */
static uint32_t capture_modifiers(struct slurp_seat *seat) {
    if (!seat->xkb_state) return 0;
    return (xkb_state_mod_name_is_active(seat->xkb_state, XKB_MOD_NAME_SHIFT, XKB_STATE_MODS_EFFECTIVE)>0 ? 1 : 0)
        | (xkb_state_mod_name_is_active(seat->xkb_state, XKB_MOD_NAME_CTRL, XKB_STATE_MODS_EFFECTIVE)>0 ? 4 : 0);
}
...
static void handle_selection_start(struct slurp_seat *seat,
	...
	if (state->single_point) {
		...
		state->result_modifiers = capture_modifiers(seat);
		state->running = false;
...
static void print_formatted_result(FILE *stream, struct slurp_state *state , const char *format) {
	...
	for (size_t i = 0; format[i] != '\0'; i++) {
		...
		if (c == '%') {
			...
			switch (next) {
			case 'M':
				fprintf(stream, "%u", state->result_modifiers);
				continue;
			case 'x':
