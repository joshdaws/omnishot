...
class PinControls:
    def __init__(self,pin):
        ...
        self.lock=QPushButton(pin);ui.set(self.lock,"setIconSize",QSize(15,15));self.lock.clicked.connect(lambda:pin.set_clickthrough(not pin.locked))
        ...
        self.unlock.setIcon(icon('lock'));ui.set(self.unlock,"setIconSize",QSize(15,15));self.unlock.setAccessibleName('Unlock pinned screenshot');self.unlock.setToolTip('Unlock pinned screenshot')
        ...
        ui.set(self.scale,"setStyleSheet",'QLabel { background:palette(window); color:palette(window-text); border-radius:8px; padding:2px 5px; font-size:10px; }')
        self.drag=DragHandle(pin);self.drag.setParent(pin);self.drag.setText('⋮ ▱ ⋮');ui.set(self.drag,"setFixedSize",52,22)
        self.drag.setToolTip('Drag Me — drag this image into another app. Hold Alt to keep it pinned.')
        ui.set(self.drag,"setStyleSheet",'QPushButton { background:palette(window); color:palette(window-text); border:1px solid palette(mid); border-radius:6px; padding:0; } QPushButton:hover { background:palette(light); }')
        for button in (self.close,self.lock,self.unlock):
            button.setFocusPolicy(Qt.FocusPolicy.NoFocus)
            ui.set(button,"setStyleSheet",'QPushButton { background:palette(window); color:palette(window-text); border:0; border-radius:10px; padding:0; } QPushButton:hover { background:palette(light); }')
        self.layout();self.sync()

    def layout(self):
        p=self.pin;small=min(p.width(),p.height());margin=ui.px(6) if small>=ui.px(60) else 2;size=max(8,min(ui.px(22),(small-4)//2))
