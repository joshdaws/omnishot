...
try:
    ...
    baseline,full=measure('full');assert full['rms']>.04,full
    enter(editor.audio_volume,50);assert editor.volume.value()==50
    half_data,half=measure('half');ratio=half['rms']/full['rms'];assert .46<ratio<.54,(full,half)
    click(editor.audio_muted);assert editor.audio.isMuted()
    muted_data,muted=measure('muted');assert muted['rms']<1e-6,muted
    click(editor.audio_mono);wait(lambda:editor.audio_preview.ready and editor.audio_preview.player.isSeekable())
    click(editor.audio_muted);mono_data,mono=measure('mono');assert mono['rms']>.01,mono
    difference=float(np.max(np.abs(mono_data[:,0]-mono_data[:,1])));assert difference<1e-5,difference
    choose(editor.audio_track,1);assert editor.player.activeAudioTrack()==1 and editor.audio_preview.player.activeAudioTrack()==1
    track_data,track=measure('track-2');frequencies=np.fft.rfftfreq(len(track_data),1/48000);spectrum=np.abs(np.fft.rfft(track_data[:,0]));peak=float(frequencies[np.argmax(spectrum)]);assert abs(peak-1320)<8,peak
    ...
    drift=abs(editor.player.position()-editor.audio_preview.player.position());assert drift<230,drift;editor.player.pause()
    editor.cuts=[[1.2,2.4]];editor.player.setPosition(1500);editor.player.play()
    wait(lambda:editor.player.position()>=2400)
    QTest.qWait(150);cut_drift=abs(editor.player.position()-editor.audio_preview.player.position());assert cut_drift<230,cut_drift
    editor.player.pause();assert editor.audio_preview.player.playbackState()==QMediaPlayer.PlaybackState.PausedState
    editor.cuts=[]
    ...
    probe=json.loads(backend.run(['ffprobe','-v','error','-select_streams','a','-show_entries','stream=channels','-of','json',out/'edited.mp4']))['streams'];assert [t['channels'] for t in probe]==[1,1]
    ...
    expected=editor.edit_options();temporary=editor.audio_preview.directory.name;editor.close();assert not Path(temporary).exists()
    ...
    assert editor.edit_options()==expected and editor.audio_volume.value()==50 and editor.audio_mono.isChecked()
    editor.close()
    report=dict(native_audio_controls=True,isolated_loopback=True,physical_microphone_used=False,full=full,half=half,gain_ratio=ratio,muted=muted,mono=mono,mono_channel_difference=difference,second_track_peak_hz=peak,speed_seek_drift_ms=drift,cut_skip_drift_ms=cut_drift,gif_preserves_audio_edits=True,native_export_mono_tracks=probe,native_editable_save_reopen=True,temporary_preview_cleanup=True)
